...
$(document).ready(function () {
    ...
    require(["esri/map", "dojo/parser", "esri/layers/FeatureLayer", "esri/config", "esri/symbols/SimpleMarkerSymbol", "esri/symbols/SimpleLineSymbol", "esri/symbols/SimpleFillSymbol", "dojo/_base/array"
        ...
        , LabelLayer) {
        $.ajax({
            url: RootUrl + 'Home/Configuration',
            type: "POST"
        }).done(function (data) {
            parser.parse();
            ...
            for (var i = 0; i < data.data.length; i++) {
                infoTemplateContent += "<br><b>" + data.data[i].Name + "</b>: ${" + data.data[i].Attribute + "}";
            }
            ...
            //Takes a URL to a non cached map service.
            transitZonesFeatures = new FeatureLayer(TransitZonesUrl, {
                visible: false,
                mode: FeatureLayer.MODE_ONDEMAND,
                outFields: ["*"]
            });
            //Takes a URL to a non cached map service.
            subwayFeatures = new FeatureLayer(SubwayUrl, {
                visible: false,
                mode: FeatureLayer.MODE_ONDEMAND,
                outFields: ["*"]
            });
            ...
            swal("Failed to load application");
        });
    });
});